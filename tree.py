...
def recursive_tree_part_recom(
    ...
    count = 0
    for part in parts[:-1]:
        min_pop = max(pop_target[count] * (1 - epsilon), pop_target[count] * (1 - epsilon) - debt)
        max_pop = min(pop_target[count] * (1 + epsilon), pop_target[count] * (1 + epsilon) - debt)
        nodes = method(
            graph.subgraph(remaining_nodes),
            pop_col=pop_col,
            pop_target=(min_pop + max_pop) / 2,
            epsilon=(max_pop - min_pop) / (2 * pop_target[count]),
        ...
        debt += part_pop - pop_target[count]
